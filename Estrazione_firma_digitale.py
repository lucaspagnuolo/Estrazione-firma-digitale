...
if uploaded:
    ...
    for uf in uploaded:
        ...
        fpath = tmp / uf.name
        fpath.write_bytes(uf.getbuffer())

        if fpath.suffix == ".zip":
            # Estrazione base + ricorsiva
            with zipfile.ZipFile(fpath, 'r') as zf:
                zf.extractall(tmp)
            recursive_unpack(tmp)
            # Copia sempre tutto in root
            for d in tmp.iterdir():
                if d.is_dir():
                    shutil.copytree(d, root / d.name, dirs_exist_ok=True)
    ...
    # Anteprima basata su root
    file_paths = []
    for base, _, files in os.walk(root):
        for f in files:
            rp = Path(base, f).relative_to(root).as_posix()
            file_paths.append(rp)

    if not file_paths:
        st.warning("⚠️ Nessun file estratto: la struttura è vuota.")
    else:
        split_paths = [p.split("/") for p in file_paths]
        max_lv = max(len(sp) for sp in split_paths)
        cols = [f"Livello {i+1}" for i in range(max_lv)]
        rows = [sp + [""] * (max_lv - len(sp)) for sp in split_paths]
        df = pd.DataFrame(rows, columns=cols)
        for c in cols:
            df[c] = df[c].mask(df[c] == df[c].shift(), "")
        st.subheader("Anteprima struttura dei file estratti")
        st.table(df)
